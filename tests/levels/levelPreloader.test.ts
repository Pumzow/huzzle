...
test("shares a pending level preload and removes it when consumed", async () => {
  ...
  const first = preloader.preload("https://example.com/levels.json", options, 1000);
  const second = preloader.preload("https://example.com/levels.json", options, 1000);

  expect(second).toBe(first);
  expect(await preloader.take("https://example.com/levels.json", options, 1000)).toEqual({
    id: 4,
    imageUrl: "https://example.com/4.webp",
  });
  expect(calls).toBe(1);

  await preloader.preload("https://example.com/levels.json", options, 1000);
  expect(calls).toBe(2);
});
